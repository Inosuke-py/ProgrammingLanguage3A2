...
- **Re-runs are safe**: existing project folders that the script manages get fully replaced with the latest snapshot, so removed files in the source repo also disappear from the aggregate. No drift.
- **Folders you add manually are never deleted**: the script keeps a manifest (`.compile-manifest.json`) of folders it placed itself, and only ever prunes from that list. If you `mkdir Some-Folder` inside the aggregate by hand, the script will commit and push it (since `git add -A` picks it up) but never delete it on subsequent runs.
